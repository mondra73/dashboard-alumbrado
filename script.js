...
function setDatos(regs) {
  ...
  for(let i=0;i<N;i++){
    const r=regs[i];
    lat[i]=r[0];lon[i]=r[1];cls[i]=r[2];stx[i]=r[3];alt[i]=r[4];tpo[i]=r[5];yr[i]=r[6];zon[i]=r[7]||0;
    tec[i]=clsTec[cls[i]];
...
function renderViewSummary(vt,vl,vo,vb){
  document.getElementById("vTot").textContent=nf.format(vt);document.getElementById("vLed").textContent=nf.format(vl);document.getElementById("vOld").textContent=nf.format(vo);
...
function renderLegend(){const list=mode==="bin"?BINS:TECS,host=document.getElementById("legend");host.innerHTML="";const totals=list.map(()=>0);for(let i=0;i<N;i++)if(inZ(i))totals[list.findIndex(l=>l.k===grp(i))]++;list.forEach((l,ix)=>{const b=document.createElement("button");b.className="leg";b.style.color=l.col;const isOn=mode==="bin"?(l.k===0?on[0]:on.slice(1).some(Boolean)):on[l.k];b.setAttribute("aria-pressed",isOn);b.innerHTML='<i class="dot"></i><span class="nm">'+l.label+'</span><span class="qt">'+nf.format(totals[ix])+'</span>';b.onclick=()=>{if(mode==="bin"){if(l.k===0)on[0]=!on[0];else{const v=!on.slice(1).some(Boolean);on[1]=on[2]=on[3]=on[4]=v;}}else on[l.k]=!on[l.k];if(!on.some(Boolean))on=[true,true,true,true,true];renderLegend();redraw();};host.appendChild(b);});}

function renderSearch(){const host=document.getElementById("qres"),raw=query.trim();if(raw.length<2){host.innerHTML="";return;}const m=raw.match(/^(.*?)[\s,]*(\d{1,6})?$/),nom=(m[1]||raw).trim().toLowerCase(),alt0=m[2]?+m[2]:null,hits=[];calles.forEach((nm,si)=>{if(nom&&!nm.toLowerCase().includes(nom))return;stIdx[si].forEach(i=>hits.push(i));});if(!hits.length){host.innerHTML='<p class="hint">Ninguna calle coincide.</p>';return;}hits.sort((a,b)=>alt0!==null?Math.abs(alt[a]-alt0)-Math.abs(alt[b]-alt0)||alt[a]-alt[b]:alt[a]-alt[b]);const top=hits.slice(0,12);host.innerHTML='<div class="res"></div><p class="hint">'+nf.format(hits.length)+' lámparas coinciden'+(alt0!==null?'. Ordenadas por cercanía a la altura '+nf.format(alt0)+'.':'. Escribí también la altura para afinar.')+'</p>';const res=host.querySelector(".res");top.forEach(i=>{const b=document.createElement("button");b.className="rw";b.style.color=colorOf(i);b.innerHTML='<i class="dot"></i><span class="rw-n">'+calles[stx[i]]+(alt[i]?" "+nf.format(alt[i]):"")+'<span class="rw-s">'+clases[cls[i]]+' · '+barrios[bar[i]].nm+'</span></span>';b.onclick=()=>{if(!map||!map.setView)return;map.setView([lat[i],lon[i]],19);setTimeout(()=>{L.popup().setLatLng([lat[i],lon[i]]).setContent('<b style="color:'+colorOf(i)+'">'+clases[cls[i]]+'</b><br>'+calles[stx[i]]+(alt[i]?" "+nf.format(alt[i]):"")+'<br><span class="dim">'+(tec[i]?"Tecnología anterior":"LED")+' · '+barrios[bar[i]].nm+' (aprox.)</span>').openOn(map);},260);};res.appendChild(b);});}
